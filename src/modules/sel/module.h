 ...
 */
void sSelectModules(const std::vector<sModuleDef>& gfx_mods, char** sel_impl);
#ifdef _WIN32
...
void sSelectModules(const std::vector<sModuleDef>& gfx_mods, char** sel_impl) {
    ...
    HWND label = CreateWindow("STATIC", "Select Graphics Module:", WS_VISIBLE | WS_CHILD, 10, 10, 160, 20, hwnd, NULL, wc.hInstance, NULL);
